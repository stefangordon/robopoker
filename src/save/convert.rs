...
impl Converter {
    ...
    pub fn convert_blueprint_to_postgres() {
        ...
        if !old_blueprint_path.exists() {
            log::error!(
                "Blueprint file not found at: {}",
                old_blueprint_path.display()
            );
    ...
    fn convert_mmap_to_postgres(source_path: &str) {
        ...
        if mmap.len() < MMAP_HEADER_SIZE {
            panic!(
                "Source file too small to contain header: {} bytes",
                mmap.len()
            );
        }
        ...
        if mmap.len() < expected_size {
            panic!(
                "Source file size mismatch: expected {} bytes, got {}",
                expected_size,
                mmap.len()
            );
        }
        ...
        log::info!(
            "Converting {} records from mmap to PostgreSQL format",
            total_records
        );
        ...
        let output_file =
            File::create(&output_path).expect("Failed to create output file blueprint.pg");

        let mut encoder = ZEncoder::new(output_file, 3).expect("Failed to create zstd encoder");
    ...
    fn write_pg_header<W: Write>(writer: &mut W) {
        ...
        writer
            .write_all(b"PGCOPY\n\xff\r\n\0")
            .expect("Failed to write PostgreSQL signature");
        // Flags field (32-bit integer, no OIDs)
        writer
            .write_u32::<BigEndian>(0)
            .expect("Failed to write flags field");
        // Header extension length (32-bit integer, no extensions)
        writer
            .write_u32::<BigEndian>(0)
            .expect("Failed to write header extension length");
    }
    ...
    fn write_pg_record<W: Write>(
        ...
        writer
            .write_u16::<BigEndian>(6)
            .expect("Failed to write field count");

        // Field 1: past (history) - 8 bytes
        writer
            .write_u32::<BigEndian>(8)
            .expect("Failed to write field length"); // field length
        writer
            .write_u64::<BigEndian>(history)
        ...
        writer
            ...
            .expect("Failed to write field length"); // field length
        writer
            .write_u64::<BigEndian>(futures)
            .expect("Failed to write futures");
        ...
        writer
            ...
            .expect("Failed to write field length"); // field length
        writer
            .write_f32::<BigEndian>(regret)
            .expect("Failed to write regret");
    ...
    fn write_pg_trailer<W: Write>(writer: &mut W) {
        ...
        writer
            ...
            .expect("Failed to write PostgreSQL trailer");
    }
    ...
    fn convert_mmap_to_parquet(source_path: &str) {
        ...
        log::info!(
            "Converting {} records from mmap to Parquet format",
            total_records
        );
        ...
        let options = WriteOptions {
            ...
            )), // Fast ZSTD compression level 1compression: CompressionOptions::Snappy, // Fast compression with good compatibility
            version: Version::V2,
